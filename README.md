...
## gf-admin-company-modules 多机构租户企业业务框架

gf-admin-company-modules 是一个专门面向多机构租户场景的企业级业务框架。该框架提供了完整的多主体业务支持，包括：
...
- 支持国际化
- 支持自动生成DAO，API
- 支持无侵入式数据库查询缓存，自动刷新缓存
- 支持自定义表模型，无感自动映射至底层逻辑服务应用
- 支持自定义钩子方法
- 支持通过钩子跨进程、跨服务器worker 跨进程通信
