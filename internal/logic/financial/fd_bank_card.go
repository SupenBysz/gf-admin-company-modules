...
func NewFdBankCard(modules co_interface.IModules) co_interface.IFdBankCard {
	return &sFdBankCard{
		modules: modules,
		dao:     modules.Dao(),
	}
...
func (s *sFdBankCard) GetBankCardByCardNumber(ctx context.Context, cardNumber string) (*co_entity.FdBankCard, error) {
	...
	err := s.dao.FdBankCard.Ctx(ctx).Where(co_do.FdBankCard{CardNumber: cardNumber}).Scan(&bankCard)
	if err != nil {
